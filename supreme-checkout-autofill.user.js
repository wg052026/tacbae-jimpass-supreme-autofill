...
(function () {
  ...
  // 브라우저 자동완성 등이 값을 덮어써도, 일정 시간 동안 우리 값으로 계속 되돌린다.
  const ENFORCE_DURATION_MS = 30000;
  const ENFORCE_INTERVAL_MS = 400;
  ...
  async function run() {
    ...
    if (location.hostname === "shop.supreme.com") {
      ...
      const krFields = buildKrFields();
      await runGenericAutofill(krFields);
      sendDiag({ step: "kr_autofill_done" });
      startEnforcing(krFields);
      ...
      const usFields = buildUsFields(profile);
  ...
  function createProfileSwitcher() {
    ...
    wrap.style.cssText =
      "position:fixed;bottom:130px;right:16px;z-index:2147483647;background:#1a1a1a;" +
